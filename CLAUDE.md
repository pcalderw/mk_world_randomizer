...
- `#tryResume()` is a stub that always returns `false` — tournament persistence/resume across reloads is a documented TODO, not implemented.

**Persistence:** `src/CourseRepo.js` wraps `localStorage` to track which courses were recently presented/selected, so `Randomizer.js` can avoid repeats. Note `updateInProgressOptions`/`updateInProgressSelection` reference an undefined global `mkw_inprogress_courses` instead of the `ONGOING_TOURNAMENT_KEY` constant — these two methods are effectively dead/broken code, unused by `TournamentManager`.
...
**Vestigial Jekyll-era code:** `_layouts/default.html` still loads `idb` (IndexedDB) from a CDN and a script at `assets/js/repo.js` that doesn't exist in this repo, plus injects `window.mkw = {courses, karts}` from Jekyll data files — all leftovers from before the "Converted to a react app" commit (6fffa62). The React app does not use any of this; it imports its own copies from `src/constants/`.
